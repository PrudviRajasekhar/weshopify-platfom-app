...
import com.weshopify.platform.features.customers.errors.CustomerNotFoundException;
import com.weshopify.platform.features.customers.errors.CustomerUpdateException;
...
public class CustomerServiceImpl implements CustomerService {
	...
	public CustomerBean saveCustomer(CustomerBean customerBean) {
		...
		try {
			...
			if(e instanceof DataAccessException) {
				String causeOfException = ((DataAccessException) e).getMostSpecificCause().getMessage();
				throw new CustomerUpdateException(causeOfException);
			}else {
				throw new CustomerUpdateException(e.getLocalizedMessage());
			}
	...
	public CustomerBean findCustomerById(int customerId) {
		...
		opt.ifPresentOrElse((customer) -> {
			...
			BeanUtils.copyProperties(customer, customerBean);
				...
				() -> {
					throw new CustomerNotFoundException("No Customer Found with the CustomerId: " + customerId);
				});
	...
	@Transactional(isolation = Isolation.READ_COMMITTED, propagation = Propagation.NOT_SUPPORTED, rollbackFor = Throwable.class)
